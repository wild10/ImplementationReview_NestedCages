...
// calcular la matriz que convierte gradiente en puntos de cuadratura 
// a gradientes en vertices del mesh
...
void gradQ_to_gradV( const Eigen::MatrixXd & V0,  const Eigen::MatrixXi & F0,  const Eigen::VectorXd & area_0,   const int quad_order,  Eigen::SparseMatrix<double> & A){
  ...
  if (quad_order==1)
  {
    ...
    for (int k = 0;k < F0.rows(); k++){
      ...
      assert(F0(k,0)<V0.rows() && F0(k,1)<V0.rows() && F0(k,2)<V0.rows()); // si es igual a 0 devuelve false
      assert(k<F0.rows());
    ...
    for (int k = 0; k < F0.rows(); k++){
      // cojemos columnas de la fila k y sacamos area
  ...
  } else if (quad_order==3)
  {
    ...
    {
      assert(false && "quad orden deberia ser 1, 2, or 3");
      cout << "gradQ_to_gradV, ordern de cuadratura " << quad_order << " no esta implementado solo es hasta 3"  << endl;
    }
...
void grad_energy(const Eigen::MatrixXd & V, const Eigen::MatrixXi & F, const Eigen::MatrixXd & V_coarse, 
  ...
  Eigen::MatrixXd & grad)
{
  ...
  int quad_order = [&F,&A_qv]()->int
  {
    ...
    {
      assert(false && "#A_qv deberia ser multiplo de #F");
      return -1;
    }
  }();
  ...
  } else if (quad_order==2)
  {
    ...
    // concatenando cuadratura de puntos
  ...
  } else if (quad_order==3)
  {
    ...
    // concatenar cuadratura de puntos
  ...
  {
    assert(false && "quad_order deberia ser 1, 2, or 3");
    cout << "grad_energy, ordern de cuadratura " << quad_order << " no esta implementado "  << endl;
  }
...
bool flow_fine_inside_coarse(const Eigen::MatrixXd & V0, const Eigen::MatrixXi & F0, const Eigen::MatrixXd & V_coarse, const Eigen::MatrixXi & F_coarse, const Eigen::SparseMatrix<double> & A_qv,
  ...
  //Si bien hay intersecciones o algún negativo widding number, sigue fluyendo
  ...
  MatrixXd V = V0;
  H.push(V);   //  Inicializamos history con input mesh vertex
  // calculamos el diametro del mesh para el tamaño de escala de paso
  ...
  VectorXd W(1); // winding number  del primer punto
  ...
  while (IF.rows() > 0 || W[0] < 1e-10){
    ...
    cout << "Flow paso " << step << ":interseccion de Coarse y fine mesh  " << endl;
    flow_one_step(MatrixXd(V), F0, V_coarse, F_coarse, A_qv, M_inv, delta_t, V);
    intersect_other(V,F0,V_coarse,F_coarse,true,IF);
    step = step+1;
    // **Alec: notar que no podemos pasar V como input y output, en vez de eso envuelve el
    // input dentro de MatrixXd para forzar al compilador a hacer una copia en este caso.
    winding_number(V_coarse,F_coarse,V.row(0),W);
    H.push(V);
    // salir despues de  1000 paso de iteracion de flujo
